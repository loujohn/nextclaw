...
export class ContextBuilder {
  ...
  private getIdentity(messageToolHints?: string[]): string {
    ...
    const lines = [
      ...
      "- Config files (MEMORY.md, AGENTS.md, TOOLS.md, BOOT.md, HEARTBEAT.md, SOUL.md, IDENTITY.md, USER.md) → write directly to the workspace root.",
      "- All other output files (code, reports, documents, data files, etc.) → write to the `files/` subdirectory (e.g., `files/hello.py`, `files/report.xlsx`). This keeps the workspace root clean.",
      "",
