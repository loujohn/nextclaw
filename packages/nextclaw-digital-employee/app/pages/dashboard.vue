...
const employeeStatsMap = computed(() => {
  const m = new Map<
    string,
    { todayRunCount: number; totalRunCount: number; todaySuccessRate: number }
  >();
...
function switchEmployee(delta: number) {
  ...
  currentEmployeeIndex.value =
    (((currentEmployeeIndex.value + delta) % len) + len) % len;
}
...
>(() => {
  ...
  return [
    {
      icon: Bot,
      iconColor: "bg-indigo-50 ring-indigo-200/50 text-indigo-500",
      label: "数字员工",
    ...
    {
      icon: CheckCircle2,
      iconColor: "bg-emerald-50 ring-emerald-200/50 text-emerald-500",
      label: "今日任务",
      value: s.todayRunCount,
      unit: "次",
      barPercent: Math.min(s.todayRunCount * 10, 100),
    ...
    },
    {
      ...
      iconColor: "bg-emerald-50 ring-emerald-200/50 text-emerald-500",
      label: "执行成功率",
      value: s.todaySuccessRate,
      unit: "%",
      barPercent: s.todaySuccessRate,
      barColor: "bg-emerald-500",
    },
    {
      icon: Zap,
      iconColor: "bg-blue-50 ring-blue-200/50 text-blue-500",
      label: "平台技能",
      value: s.totalSkillCount,
      unit: "个",
      barPercent: Math.min(s.totalSkillCount * 5, 100),
...
async function openRunDetail(runId: string) {
  ...
  try {
    const payload = await $fetch<{ ok: boolean; data: RunDetail }>(
      `/api/runs/${runId}`,
    );
...
<template>
  ...
  <div v-else class="p-7 mx-auto space-y-5 max-w-6xl lg:p-9">
    <!-- Layer 1: Header -->
    <header class="relative">
      <span
        class="inline-block text-[11px] font-bold uppercase tracking-wider text-primary bg-primary/8 px-2.5 py-[3px] rounded-md mb-2"
      >
        工作中心
      </span>
      <h1
        class="text-[28px] font-extrabold tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text"
      >
        平台运行全景
      </h1>
      <p
        class="text-[13px] text-muted-foreground/80 leading-relaxed mt-1.5 max-w-xl"
      >
    ...
    <div class="grid gap-4" style="grid-template-columns: 320px 1fr">
      ...
      <div
        class="rounded-2xl border border-border/60 bg-card overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.03)]"
      >
        <div
          class="flex items-center justify-between px-[18px] py-3.5 border-b border-border/60"
        >
          <h3 class="text-sm font-bold">数字员工</h3>
          <div class="flex gap-2 items-center">
            <template v-if="hasMultipleEmployees">
              <button
                class="flex justify-center items-center w-5 h-5 text-xs rounded transition-colors text-muted-foreground hover:bg-muted"
            ...
            <span
              class="text-[10px] font-semibold text-primary bg-primary/10 px-2 py-[2px] rounded-md"
            >
        ...
        <div class="p-[18px]">
          <DashboardEmployeeCard
            ...
            :dept-name="
              currentEmployee.departmentId
                ? (deptNameMap.get(currentEmployee.departmentId) ?? null)
                : null
            "
          ...
          <div v-else class="flex flex-col items-center py-6">
            <p class="mb-3 text-xs text-muted-foreground">还没有数字员工</p>
            <NuxtLink
      ...
      >
        ...
        >
          ...
          <span
            class="flex items-center gap-1.5 text-[10px] font-semibold text-primary bg-primary/8 px-2 py-[2px] rounded-md"
          >
            <span class="w-1.5 h-1.5 rounded-full animate-pulse bg-primary" />
            实时更新
          </span>
        </div>
        <div class="px-[18px] py-3.5">
          <DashboardOutputFeed @click-run="goToRun" />
        </div>
      </div>
    </div>
