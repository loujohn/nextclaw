...
class APIClient:
    ...
    def _fetch_json_get(self, url, timeout=120):
        """GET 请求"""
        headers = {
            "Accept": "application/json",
    ...
    def query_daily_reports(self, week_start, week_end, username=None, day_report_type=2, page=1, size=50):
        """查询日报数据"""
        params = {
        ...
        url = f"{self.base_url}/admin/day/report/page?{urllib.parse.urlencode(params)}"
        return self._fetch_json_get(url)
    ...
    def query_users_by_dept(self, dept_id, size=200):
        """按部门查询用户"""
        url = f"{self.base_url}/admin/user/page?current=1&size={size}&deptId={dept_id}"
        return self._fetch_json_get(url)
    ...
    def submit_weekly_report(self, report_data, report_user=None):
        """提交周报"""
        url = f"{self.base_url}/admin/weekReport"
        params = None
        if report_user:
