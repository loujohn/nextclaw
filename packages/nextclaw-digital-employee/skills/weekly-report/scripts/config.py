...
class Config:
    ...
    # API 配置
    BASE_URL = os.environ.get("DW_PM_BASE_URL", "")
    BASIC_AUTH = os.environ.get("DW_PM_BASIC_AUTH", "")
    USERNAME = os.environ.get("DW_PM_USERNAME", "")
    PASSWORD = os.environ.get("DW_PM_PASSWORD", "")
    ...
    def validate(cls):
        ...
        missing = []
        if not cls.BASE_URL:
            missing.append("DW_PM_BASE_URL")
        if not cls.USERNAME:
            missing.append("DW_PM_USERNAME")
        if not cls.PASSWORD:
            missing.append("DW_PM_PASSWORD")
        if missing:
