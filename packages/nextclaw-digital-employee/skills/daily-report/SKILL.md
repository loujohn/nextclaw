...
**注意：必须先确认填报人信息，再开始其他操作！**

1. **获取并确认填报人** → 从钉钉会话上下文中获取填报人，与用户确认无误后，方可继续
2. **收集必填信息** → 从用户输入中提取：日期、今日总结、明日计划
3. **查询项目** → 询问用户要填哪个项目，调用 `--query-projects` 查询并让用户选择
4. **确认并提交** → 用户确认后，调用 `--submit` 提交
...
**强制确认**：获取后必须与用户确认："当前填报人为 XXX（username），是否正确？"

- 用户确认正确 → 继续后续操作
- 用户反馈有误/上下文中无法获取 → 询问用户提供正确信息，确认后再继续
...
| `--report-user`          | 填报人用户名（优先从会话获取）|
| `--report-name`          | 填报人中文名（优先从会话获取）|
...
**注：** 
- `--report-user` 和 `--report-name` 用于指定填报人
- 当通过数字员工对话调用时，这两个参数优先从会话信息自动获取
- 仅在会话信息缺失时才需要用户手动提供
